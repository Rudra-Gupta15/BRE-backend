...
| Pipeline (Model Hub) | `GET /pipeline/uploads`, `POST /pipeline/uploads` (multipart `files` folder, or single `file`), `POST /pipeline/uploads/autofill`, `POST /pipeline/run`, `GET /pipeline/status` |
...
- **`POST /pipeline/uploads`** accepts a whole *folder* of real files for one `sourceId` (repeated `files` parts; a single `file` part is still accepted for the Model Testing page — with `scope=testing` it goes to a slot separate from Model Hub's). Each file is scanned + parsed independently and the set is stored as a list — `session_state.uploaded_files[sid]` / `parsed_statements[sid]` are lists, one entry per file — so training runs across every file. `services/file_analysis.py` scans the bytes for a cleanliness score, then the router overrides it with parse completeness (0 transactions → 10%). `services/statement_parser.py` extracts real transactions per extension: **PDF** (`pymupdf` text → vision LLM fallback), **CSV/TSV/TXT** (column parser: tab/comma/semicolon/pipe delimiters, free-text heuristic fallback), **JSON** (fuzzy field-matching — array / `{transactions:[…]}` / nested AA payload), **MD** (pipe-table → column parser), **XLSX** (first sheet via `pandas` + `openpyxl` → column parser). Allowed extensions: `pdf,csv,tsv,txt,json,md,xlsx` (`ALLOWED_UPLOAD_EXT`). Unsupported files in a folder are recorded with an error, not fatal. The pipeline engine and inference collapse a source's folder into one merged statement via `session_state.merged_statement_for(sid)`.
