# bre_engine.py
#
# Evaluates the Business Rule Engine (BRE) rules that are enabled on the
# Settings page against a real applicant's bank statement + derived feature
# vector + credit score, and returns a PASS / FAIL / SKIP verdict per rule plus
# an overall decision.
#
# "PASS"  = the rule evaluated favourably for the applicant (no concern)
# "FAIL"  = the rule's risk condition triggered (a concern)
# "SKIP"  = enabled, but not computable from a single bank statement
#           (needs an external feed — GST turnover, bureau — or 6-12 months)
...
def _build_context(fv: dict, risk: dict, transactions: list[dict], opening_balance: float | None) -> dict:
...
def evaluate_bre_rules(
    ...
    c = _build_context(fv, risk, transactions, opening_balance)
