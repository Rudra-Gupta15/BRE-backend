"""AA session ML — trains the 4 bank models from the uploaded statement.

Real scikit-learn, no faked numbers:
  1. Extract genuine financial features from the parsed statement (ADB,
     credit/debit ratio, balance volatility, transaction velocity, ...).
  2. Build a synthetic 600-applicant population anchored to those features so
     the training set has variance without needing historical loan data.
  3. Train 4 models with 5-fold cross-validation:
  ...
  4. Report real cross-validation accuracy.
  5. Keep the fitted estimators in memory (models_state) for live inference.

Called by the /models/train route. GST training is the separate app.gst.model.
"""
...
def _golden_accuracy(artifact: dict) -> float | None:
    ...
    from app.common.security import poisoning

    gdf = poisoning.load_golden_set()
    if gdf is None or len(gdf) < 30:
        return None
    try:
        from app.aa import dataset as dataset_store
