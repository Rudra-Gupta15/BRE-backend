...
from app.aa.catalog import ML_ALGORITHMS, VERSION_OPTIONS
from app.aa import dataset as dataset_store, registry as model_registry
from app.aa.model import train_models_live, train_on_dataset
...
from app.common.security import lineage, poisoning
...
async def train_models_handler(body: TrainBody):
    ...
    try:
        ...
        pat = patterns.train_pattern_models()
        models_state.evaluation_cache = {**models_state.evaluation_cache,
                                         **{k: {"evalMetrics": v["evalMetrics"], "cvFolds": v["cvFolds"]}
                                            for k, v in pat.items()}}
    except Exception:  # noqa: BLE001
        logger.warning("AA pattern-model training failed", exc_info=True)
...
async def evaluation_summary():
    ...
    from app.aa.patterns import _PATTERN_MODEL_NAMES

    session_models = []
    for mid, name in {**_MODEL_NAMES, **_PATTERN_MODEL_NAMES}.items():
        ev = cache.get(mid)
        if not ev:
            continue
        ...
        session_models.append({
            ...
            "kind": "pattern" if mid in _PATTERN_MODEL_NAMES else "bank",
        })
...
async def rerun_evaluation(model_id: str):
    ...
    if model_id in PATTERN_MODEL_IDS:
        loop = asyncio.get_event_loop()
        pat = await loop.run_in_executor(None, train_pattern_models)
        entry = pat.get(model_id)
        if not entry:
            raise HTTPException(400, "Train the bank models first.")
        models_state.evaluation_cache = {
            **models_state.evaluation_cache,
            **{k: {"evalMetrics": v["evalMetrics"], "cvFolds": v["cvFolds"]} for k, v in pat.items()},
        }
    ...
    from app.aa.model import evaluate_trained
    ...
    from app.aa.model import _build_dataset, _build_labels
    import numpy as np
...
async def get_patterns():
    """Post-training pattern recognition for the bank models — behavioral
    patterns in the uploaded statement(s) + what each trained model weights.
    Rendered in the Model Hub's "Detected Patterns" card."""
    from app.aa import patterns
