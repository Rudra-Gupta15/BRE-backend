...
Idempotent: re-running it will not create duplicates.
    ...
    ./venv/Scripts/python.exe backfill_from_cache.py
"""
import json
import re
...
from app.models_db import Applicant, InferenceRun
from app.services.persistence import save_inference_run, save_statement

CACHE = Path(__file__).with_name(".session_cache.json")
...
def main() -> None:
    ...
    for source_id, ps in parsed.items():
        ...
        if stmt_id:
            ...
            from app.routers.inference import _build_bundle
